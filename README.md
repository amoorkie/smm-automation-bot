...
Бот забирает рабочие фото и контентные темы из Supabase, собирает сценарий публикации, генерирует текст и визуалы через OpenRouter и возвращает оператору готовый preview прямо в Telegram. Весь рабочий цикл проходит в одном интерфейсе: от выбора режима до ревизий, подтверждения публикации и отправки готового материала в VK.
...
- `VK API` для публикации подтвержденных материалов в группу;
